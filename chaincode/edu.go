...
type ChaincodeInfo struct {
	...
	SignDate	string	`json:"SignDate"`		// 性别
	Nation	string	`json:"Nation"`		// 民族
	CopyRightID	string	`json:"CopyRightID"`		// 证书编号
	Creator	string	`json:"Creator"`		// 籍贯
	Holder	string	`json:"Holder"`		// 颁布日期

	EnrollDate	string	`json:"EnrollDate"`		// 
	NowState	string	`json:"NowState"`	// 
	DoneDate	string	`json:"DoneDate"`	// 
	Major	string	`json:"Major"`	// 
	QuaType	string	`json:"QuaType"`	// 认证方式
	Length	string	`json:"Length"`	// 有效期
	Mode	string	`json:"Mode"`	// 
	Level	string	`json:"Level"`	// 层次
	CopyRightLevel	string	`json:"CopyRightLevel"`	// 
...
func (t *EducationChaincode) Invoke(stub shim.ChaincodeStubInterface) peer.Response{
	...
	if fun == "addEdu"{
		...
		return t.queryEduByCertNoAndName(stub, args)		// 根据身份证号及姓名查询信息
	}else if fun == "queryEduInfoByCopyRightID" {
		return t.queryEduInfoByCopyRightID(stub, args)	// 根据证书编号及姓名查询详情
	}else if fun == "updateEdu" {
		return t.updateEdu(stub, args)		// 根据身份证号更新信息
	}else if fun == "delEdu"{
...
// 保存edu
// args: ChaincodeInfo
func PutEdu(stub shim.ChaincodeStubInterface, edu ChaincodeInfo) ([]byte, bool) {
	...
	// 保存edu状态
	err = stub.PutState(edu.CopyRightID, b)
	if err != nil {
		return nil, false
	}
...
// 根据证书编号查询信息状态
// args: CopyRightID
func GetEduInfo(stub shim.ChaincodeStubInterface, CopyRightID string) (ChaincodeInfo, bool)  {
	var edu ChaincodeInfo
	// 根据证书编号查询信息状态
	b, err := stub.GetState(CopyRightID)
	if err != nil {
		return edu, false
	}
...
// 证书编号为 key, ChaincodeInfo 为 value
func (t *EducationChaincode) addEdu(stub shim.ChaincodeStubInterface, args []string) peer.Response {
	...
	// 查重: 证书编号必须唯一
	_, exist := GetEduInfo(stub, edu.CopyRightID)
	if exist {
...
func (t *EducationChaincode) queryEduByCertNoAndName(stub shim.ChaincodeStubInterface, args []string) peer.Response {
	...
	if err != nil {
		return shim.Error("根据身份证号及姓名查询信息时发生错误")
	}
	if result == nil {
		return shim.Error("根据指定的身份证号及姓名没有查询到相关的信息")
	}
...
// 根据证书编号查询详情（溯源）
// args: CopyRightID
func (t *EducationChaincode) queryEduInfoByCopyRightID(stub shim.ChaincodeStubInterface, args []string) peer.Response {
	if len(args) != 1 {
		return shim.Error("给定的参数个数不符合要求")
	}
	...
	if err != nil {
		return shim.Error("根据证书编号查询信息失败")
	}
	...
	// 获取历史变更数据
	iterator, err := stub.GetHistoryForKey(edu.CopyRightID)
	if err != nil {
	...
	for iterator.HasNext() {
		...
		if hisData.Value == nil {
			var empty ChaincodeInfo
			historyItem.ChaincodeInfo = empty
		}else {
			historyItem.ChaincodeInfo = hisEdu
		}
...
func (t *EducationChaincode) updateEdu(stub shim.ChaincodeStubInterface, args []string) peer.Response {
	...
	// 根据证书编号查询信息
	result, bl := GetEduInfo(stub, info.CopyRightID)
	if !bl{
		return shim.Error("根据证书编号查询信息时发生错误")
	}

	result.Name = info.Name
	result.Holder = info.Holder
	result.Nation = info.Nation
	result.SignDate = info.SignDate
	result.Creator = info.Creator
	result.CopyRightID = info.CopyRightID
	result.Photo = info.Photo


	result.EnrollDate = info.EnrollDate
	result.NowState = info.NowState
	result.DoneDate = info.DoneDate
	result.Major = info.Major
	result.QuaType = info.QuaType
	result.Length = info.Length
	result.Mode = info.Mode
	result.Level = info.Level
	result.CopyRightLevel = info.CopyRightLevel
